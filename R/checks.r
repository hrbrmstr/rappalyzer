...
check_html <- function(site_html) {
  ...
  if (inherits(site_html, "response")) site_html <- httr::content(site_html, as="text")
  if (is.raw(site_html)) site_html <- readBin(site_html, character())

  app_html <- .pkgenv$html

  map(app_html, c("html", "main")) %>%
    map_lgl(~.x %~% site_html) %>%
    keep(`==`, TRUE) -> res
...
check_meta <- function(site_html) {
  ...
  if (inherits(site_html, "response")) site_html <- httr::content(site_html, as="parsed")
